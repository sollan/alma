...
class PCAPanel(wx.Panel):
    ...
    def __init__(self, parent):
        ...
        self.Bind(
            ...
            group_no=0: self.SelectGroupFolder(event, group_no),
            self.select_group1_button)
        ...
        self.Bind(
            wx.EVT_BUTTON,
            lambda event,
            group_no=1: self.SelectGroupFolder(event, group_no),
            self.select_group2_button)
        ...
        #################################
        self.select_group3_button = wx.Button(
            self, id=wx.ID_ANY, label="Group 3")
        self.sizer.Add(
            self.select_group3_button,
            pos=(5, 0), flag=wx.LEFT | wx.TOP, border=25)
        self.data_widgets.append(self.select_group3_button)
        self.Bind(
            wx.EVT_BUTTON, lambda event,
            group_no=2: self.SelectGroupFolder(event, group_no),
            self.select_group3_button)
        ...
        self.sizer.Add(
            self.select_group3_text,
            pos=(5, 1), flag=wx.LEFT | wx.TOP, border=25)
    ...
    def SelectGroupFolder(self, e, group_no):
        ...
        if import_dialog.ShowModal() == wx.ID_OK:
            ...
            if group_no == 0:
                ...
                self.sizer.Add(
                    self.group3_name_input,
                    pos=(5, 2), flag=wx.LEFT | wx.TOP, border=25)
                ...
                self.Bind(
                    wx.EVT_BUTTON,
                    lambda event,
                    group_no=2: self.OnGroupNameInput(event, group_no),
                    self.group3_name_input_button)
                self.sizer.Add(
                    self.group3_name_input_button,
                    pos=(5, 3), flag=wx.LEFT | wx.TOP, border=25)
            ...
            if group_no == 0:
                self.folder1 = ''
                self.select_group1_text.SetLabel(
                    "\nPlease select kinematic data folder of the "
                    "first group.")
            elif group_no == 1:
                self.folder2 = ''
                self.select_group2_text.SetLabel(
    ...
    def OnGroupNameInput(self, e, group_no):
        ...
        if group_no == 0:
            ...
            self.select_group1_text.SetLabel(
                f"\nGroup 1: {self.group_name_1}, "
                f"data found at {self.folder1}")
        elif group_no == 1:
            self.group_name_2 = str(self.group2_name_input.GetValue())
            self.select_group2_text.SetLabel(
                f"\nGroup 2: {self.group_name_2}, "
                f"data found at {self.folder2}")
        elif group_no == 2:
            self.group_name_3 = str(self.group3_name_input.GetValue())
            self.select_group3_text.SetLabel(
                f"\nGroup 3: {self.group_name_3}, "
                f"data found at {self.folder3}")
    ...
    def PCA(self, e):
        ...
        self.PCA_result_text = wx.StaticText(
            self, label="\nAnalysis completed!")
        self.sizer.Add(
