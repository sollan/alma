...
class AnalyzeStridePanel(wx.Panel):
    ...
    def __init__(self, parent):
        ...
        self.header = wx.StaticText(self, -1, "Kinematics Analysis", size=(500,100))
        ...
        self.instructions = wx.StaticText(self, -1, "Load the csv output from DeepLabCut and automatically extract stride and kinematics parameters.")
    ...
    def FirstPage(self):
        ...
        self.bodypart_label = wx.StaticText(self, label = 'Labelled bodypart to use for stride extraction:') 
        ...
        self.save_pred_button = wx.Button(self, id=wx.ID_ANY, label="Save extracted stride start and end frame numbers")
    ...
    def ImportKinematicsCSV(self, e):
        if not TEST:
            ...
            if import_dialog.ShowModal() == wx.ID_OK:
                ...
                self.df = KinematicsFunctions.treadmill_correction(self.df, self.bodyparts, self.treadmill_speed)
        try: 
    ...
    def SavePredFunc(self, e):
        ...
        with wx.FileDialog(self, 'Save current prediction as... ', \
            ...
            try:
                KinematicsFunctions.make_output(pathname, self.start_times, self.end_times, self.durations)
