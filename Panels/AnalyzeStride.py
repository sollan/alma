...
from Functions.KinematicsFunctions import estimate_speed
from Functions.KinematicsFunctions import read_file, extract_parameters
import os
import warnings
...
class AnalyzeStridePanel(wx.Panel):
    ...
    def __init__(self, parent):
        ...
        self.instructions = wx.StaticText(
            ...
            "and extract kinematic parameters.",
        ...
        self.sizer.Add(
            self.instructions,
            pos=(2, 0),
            span=(1, 5), flag=wx.LEFT, border=25)
    ...
    def Stride_UI(self):
        ...
        self.import_csv_text = wx.StaticText(
            self,
            label="Import a csv file for stride extraction and"
            "calculate kinematic parameters. "
            "\nThe file should be DeepLabCut output (or of a similar format)"
            "to ensure proper parsing!")
        self.sizer.Add(
            self.import_csv_text,
            pos=(6, 1),
            flag=wx.LEFT,
            border=25)
        ...
        self.import_folder_button = wx.Button(
            self,
            id=wx.ID_ANY,
            label="Bulk import")
        self.sizer.Add(
        ...
        self.import_folder_text = wx.StaticText(
            self,
            label="Import all csv files from a folder for stride extraction"
            " and calculate kinematics parameters. "
            "\n\nThe files should be DeepLabCut output "
            "(or of a similar format) to ensure proper parsing!"
            "\nAll recordings should have the same treadmill speed"
            " (if using manual input) or \nfollow the provided pixel "
            "to centimeter speed ratio (if using automatic speed detection).")
        self.sizer.Add(
            self.import_folder_text,
            pos=(7, 1),
            flag=wx.LEFT | wx.TOP,
            border=25)
        self.stride_widgets.append(self.import_folder_text)
        self.import_folder_button.Bind(
            wx.EVT_BUTTON,
            self.BulkImportKinematicsCSV)
        ...
        self.method_note_text = wx.StaticText(
            self,
            label="\nNote: please use semi-automated function when "
            "running kinematic analysis on a new experimental set-up."
            "\nThe pixel/frame speed and the corresponding px-to-cm "
            "speed ratio will be estimated and displayed. "
            "\nThe ratio can then be re-used for fully automated analysis, "
            "or for finding a more precise relationship, "
            "\ne.g., through a regression between the px-to-cm speed ratio"
            "at different speeds.")
        self.sizer.Add(
            self.method_note_text,
            pos=(9, 1),
            flag=wx.LEFT | wx.TOP,
            border=25)
        ...
        if import_dialog.ShowModal() == wx.ID_OK:
            ...
            try:
                if len(self.files) > 0:
                    ...
                    self.has_imported_file = False
                    self.import_csv_text.SetLabel(
                        f"Loaded {len(self.files)} csv files"
                        "found in {self.input_path}.\n")
                    self.GetParent().Layout()
    ...
    def OnMethod(self, e):
        ...
        if self.method_selection == "Semi-automated":
            self.method_note_text.SetLabel(
                ...
                "\nThe pixel/frame speed and the corresponding"
                "px-to-cm speed ratio will be estimated and displayed. "
                "\nThe ratio can then be re-used for fully automated analysis,"
                " or for finding a more precise relationship, "
                "\ne.g., through a regression between the px-to-cm "
                "speed ratio at different speeds.")
            ...
            try:
                ...
                self.cm_speed_input = wx.TextCtrl(
                    self,
                    value=str(self.cm_speed))
                self.sizer.Add(
                    self.cm_speed_input,
                    pos=(9, 2),
                    flag=wx.LEFT | wx.TOP,
                ...
                self.cm_speed_input_button = wx.Button(
                    self,
                    id=wx.ID_ANY,
                    label="Update speed (cm/s)")
                self.cm_speed_input_button.Bind(
                    wx.EVT_BUTTON,
                    self.UpdateSpeed)
                self.sizer.Add(
            ...
            self.method_note_text.SetLabel(
                "Using px to cm speed ratio: 1 pixel / frame = "
                f"{self.px_to_cm_speed_ratio} cm / s.")
            try:
            ...
            try:
                ...
                self.px_cm_ratio_input = wx.TextCtrl(
                    self,
                    value=str(self.px_to_cm_speed_ratio))
                self.sizer.Add(
                ...
                self.px_cm_ratio_input_button = wx.Button(
                    self,
                    id=wx.ID_ANY,
                    label="Update pixel-to-cm speed ratio")
                self.px_cm_ratio_input_button.Bind(
                    wx.EVT_BUTTON,
                    self.UpdateRatio)
                self.sizer.Add(
        ...
        if self.has_imported_file:
            ...
            if self.method_selection == 'Semi-automated':
                ...
                (self.est_cm_speed,
                    ...
                    self.est_px_to_cm_speed_ratio) = estimate_speed(
                        self.df, 'toe', self.cm_speed, None, self.frame_rate)

                self.method_note_text.SetLabel(
                    f"Recorded: {self.cm_speed} cm / s;\n\n"
                    "Estimated: \n"
                    f"Pixel speed: {self.est_px_speed} px / frame."
                    f"\nLength conversion: {self.est_pixels_per_cm} px / cm."
                    "\npx-to-cm speed ratio: 1 px / frame = "
                    f"{self.est_px_to_cm_speed_ratio} cm / s."
                    "\n(The ratio can then be re-used for fully automated "
                    "analysis, or for finding a more precise relationship, "
                    "\ne.g., through a regression between the"
                    "px-to-cm speed ratio at different speeds.)")
                ...
                (self.est_cm_speed,
                    ...
                    self.est_pixels_per_cm,
                    self.est_px_to_cm_speed_ratio) = estimate_speed(
                        self.df, 'toe', None, self.px_to_cm_speed_ratio,
                        self.frame_rate)

                self.method_note_text.SetLabel(
                    f"Recorded: 1 px / frame = {self.px_to_cm_speed_ratio} "
                    "cm / s."
                    f"\n\nEstimated: \nPixel speed: {self.est_px_speed} "
                    "px / frame."
                    f"\nRecording cm speed: {self.est_cm_speed} cm / s."
                    f"\nLength conversion: {self.est_pixels_per_cm} "
                    "pixels per cm.")
            ...
            if self.method_selection == 'Semi-automated':
                self.method_note_text.SetLabel(
                    f"Estimating pixel speed at {self.cm_speed} cm / s,"
                    " which will be used for all files in the folder."
                    "\nPlease make sure all recordings have the same"
                    "speed setting.")
            elif self.method_selection == 'Fully automated':

                self.method_note_text.SetLabel(
                    "Using px to cm speed ratio: "
                    f"1 pixel / frame = {self.px_to_cm_speed_ratio} cm / s."
                    "\nPlease make sure all recordings have the same set up"
                    " (e.g., distance from camera).")
    ...
    def SelectOutputFolder(self, e):
        ...
        if export_dialog.ShowModal() == wx.ID_OK:
            ...
            self.extract_parameters_text.SetLabel(
                ...
                "You can now start parameter extraction. \n\n"
                "Parameter extraction can take a few minutes per file "
                "(depending on your computer processors). Please be patient.")
        else:
            pass
    ...
    def BulkExtractParameters(self, e):
        ...
        for i, file in enumerate(self.files):
            ...
            self.GetParent().SetStatusText(
                f"Extracting parameters for file {i+1}"
                f" out of {len(self.files)}...")
            self.GetParent().Layout()
            ...
            if self.method_selection == 'Semi-automated':
                ...
                (parameters,
                    pd_dataframe_coords,
                    is_stance,
                    bodyparts) = extract_parameters(
                        self.frame_rate, self.df, self.cutoff_f, 'toe',
                ...
                (parameters,
                    pd_dataframe_coords,
                    is_stance, bodyparts) = extract_parameters(
                        self.frame_rate, self.df, self.cutoff_f, 'toe',
            ...
            parameters_truncated = KinematicsFunctions.return_ten_central(
                parameters, True, pd_dataframe_coords, bodyparts, is_stance,
                f'truncated_stickplot_{file}')
            KinematicsFunctions.make_parameters_output(
                os.path.join(
                    self.output_path,
                    f'10_continuous_strides_parameters_{file}'),
        ...
        self.GetParent().SetStatusText(
            "\nKinematic parameters have been extracted and saved to "
            f"{self.output_path}!\n")
    ...
    def SelectOutputPath(self, e):
        ...
        export_dialog = wx.FileDialog(
            self, 'Save parameter extraction results to... ',
    ...
    def ExtractParameters(self, e):
        ...
        self.GetParent().SetStatusText(
            "\nWorking hard to extract 44 kinematic parameters "
            f"for {self.filename}...\n")
        self.GetParent().Layout()
        ...
        if self.method_selection == 'Semi-automated':
            (parameters,
                pd_dataframe_coords,
                is_stance,
                bodyparts) = KinematicsFunctions.extract_parameters(
                    self.frame_rate, self.df, self.cutoff_f, 'toe',
                    cm_speed=self.cm_speed,
                    px_to_cm_speed_ratio=self.est_px_to_cm_speed_ratio)
        elif self.method_selection == 'Fully automated':
        ...
        parameters_truncated = KinematicsFunctions.return_ten_central(
            parameters, True, pd_dataframe_coords, bodyparts,
            is_stance, f'truncated_stickplot')
        KinematicsFunctions.make_parameters_output(
            os.path.join(
                os.path.dirname(self.output_path),
                f'10_continuous_strides_parameters.csv'),
                parameters_truncated)

        self.GetParent().SetStatusText(
            "\nKinematic parameters have been extracted and saved to "
            f"{self.output_path}! Ready for between-group analysis\n")
