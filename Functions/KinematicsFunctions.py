...
def estimate_speed(pd_dataframe, bodypart, cm_speed, px_to_cm_speed_ratio, frame_rate):
    ...
    px_speed,  = scipy.stats.norm.fit(x_change_filt[x_change_filt>0])
...
def find_angles(x1, y1, x_center, y_center, x2, y2):
    ...
    y2 = y2-y_center
    return [np.rad2deg(
        angle([x1[i], y1[i]], [x2[i], y2[i]])) for i in range(len(x1))]
...
def find_swing_stance(elevation_angle_change, start, end):
    ...
    '''
    try:
        # only when swing phase is detected
        # (neg-to-pos zero crossing exists in elevation angle delta)
        # stride start to swing onset = stance duration
        # swing onset to stride end = swing duration
        # since stride is marked based on the
        # consequtive stance onsets (toe y axis movement)
        stance_dur = np.where(
            np.diff(np.sign(elevation_angle_change[start:end])) > 0)[0][0]
    except IndexError:
...
def find_drag(toe_y, stance_dur, threshold, start, end):
    # threshold: % threshold for y axis loc (treadmill y level),
    # below which the toe is considered to be in contact with treadmill
...
def parse_stride_lists(x, starts, ends, first_stride, n, adjust=False):
    '''
    reshapes a continuous time series of bodypart location x
    into a list of n strides,
    each starting at starts[n] and ending at ends[n],
    with the first stride in the new list starting at starts[first_stride]
    starts / ends: index of the stride start and endpoints
    to use for parsing continuous time series x
    first_stride: index of the first stride to extract,
    based on the starts and ends lists
    ...
    if adjust:
        for n_sublist in range(len(results)):
            ...
            results[n_sublist] = [
                a - b for a, b in zip(results[n_sublist], centers)]
    return results
...
def pairwise_dtw(iterable):
    ...
    iterable: a list / array of time series
    (endpoint paths of a number of strides)
    returns: 
    ...
    for i in range(len(iterable)-1):
        for j in range(i+1, len(iterable)):
            distance, _ = fastdtw.fastdtw(
                iterable[i], iterable[j], dist=euclidean)
...
def findLongestSequence(A, k):
    ...
    window = 0      # stores the maximum number of continuous 1's found so far (including `k` zeroes)
...
def return_ten_central(pd_dataframe_parameters, plot=False, pd_dataframe_coords=None, bodyparts=None, is_stance=[], filename=''):
    valid_list = convert_to_binary(
        pd_dataframe_parameters['cycle duration (s)'])
    ...
    if plot:
        ...
        continuous_stickplot(
            pd_dataframe_coords, bodyparts,
            is_stance, x_coords, y_coords, start_frame, end_frame, filename)
...
def extract_parameters(frame_rate, pd_dataframe, cutoff_f, bodypart, cm_speed=None, px_to_cm_speed_ratio=0.4806, plot=False):
    '''
    pd_dataframe: contains raw coordinates
    (not adjusted for treadmill movement, unfiltered)
    ...
    smooth_toe_x = butterworth_filter(
        pd_dataframe['toe x'], frame_rate, cutoff_f)
    smooth_mtp_x = butterworth_filter(
        pd_dataframe['mtp x'], frame_rate, cutoff_f)
    smooth_ankle_x = butterworth_filter(
        pd_dataframe['ankle x'], frame_rate, cutoff_f)
    smooth_knee_x = butterworth_filter(
        pd_dataframe['knee x'], frame_rate, cutoff_f)
    smooth_hip_x = butterworth_filter(
        pd_dataframe['hip x'], frame_rate, cutoff_f)
    smooth_crest_x = butterworth_filter(
        pd_dataframe['iliac crest x'], frame_rate, cutoff_f)

    smooth_toe_y = butterworth_filter(
        pd_dataframe['toe y'], frame_rate, cutoff_f)
    smooth_mtp_y = butterworth_filter(
        pd_dataframe['mtp y'], frame_rate, cutoff_f)
    smooth_ankle_y = butterworth_filter(
        pd_dataframe['ankle y'], frame_rate, cutoff_f)
    smooth_knee_y = butterworth_filter(
        pd_dataframe['knee y'], frame_rate, cutoff_f)
    smooth_hip_y = butterworth_filter(
        pd_dataframe['hip y'], frame_rate, cutoff_f)
    smooth_crest_y = butterworth_filter(
        pd_dataframe['iliac crest y'], frame_rate, cutoff_f)
    ...
    if np.abs(stat.median(x_change)) < 0.5 and len(np.where(np.abs(x_change) < 2)[0])/len(x_change) > 0.68:
        ...
        # bodypart x coordinate rate of change does not 
        # meet assumption of multimodal distribution
        # instead it is centered around 0
        # step cycle cannot be detected properly

        print('Too much dragging? Please check raw data.')
        print(
            'Median of selected bodypart x coordinate '
            'change per frame: {stat.median(x_change)}')
        print(
            'Calculating a subset of parameters '
            'independent of step cycles...')
        ...
        filtered_bodypart_x = butterworth_filter(
            pd_dataframe[f'{bodypart} x'], frame_rate, 5)
        filtered_bodypart_x_change = np.diff(filtered_bodypart_x)

        # movement direction: x loc decreasing;
        # treadmill direction: x loc increasing
        ...
        gm = GaussianMixture(
            n_components=2,
            random_state=0).fit(np.array(b).reshape(-1, 1))
        # the Gaussian with larger mean
        # corresponds to y coord during stance phase
        # use this mean as threshold to detect dragging during swing phase
        stance_threshold = max(gm.means_)
        # use this plus SD (i.e., lower in space)
        # as treadmill y coord, to calculate step height
        treadmill_y = float(
            stance_threshold + np.sqrt(
                gm.covariances_[np.where(gm.means_ == stance_threshold)]))
        ...
        for i in range(len(starts)):
            ...
            stride_len = find_stride_len(
                smooth_toe_x, starts[i], ends[i])  # in pixel
            ...
            # does extracted stride seem accurate?
            # (e.g., due to mislabeled bodypart position)
            # extreme limb length
            ...
            if limb_len_max < 15 and stride_len_cm < 8 and stride_len > 0 and \
                ...
                # or use averaged v from euclidean distance
                # between every two frames?
                # (that would consider the entire trajectory of a stride,
                # including vertical movement / lift,
                ...
                drag, _ = find_drag(
                    smooth_toe_y, stance_dur_frame,
                    treadmill_y, starts[i], ends[i])
            ...
            if starts[i] not in starts_included:
                ...
                if included_index <= len(starts_included)-5:
                    ...
                    toe_xs = parse_stride_lists(
                        smooth_toe_x,
                        starts_included, ends_included,
                        included_index, 5, True)
...
def continuous_stickplot(filt_corrected_df, bodyparts, is_stance, x_coords, y_coords, start, end, filename='truncated_stickplot'):
    ...
    for t in filt_corrected_df.index[start:end]:
        if t % 2 == 0:
            ...
            toe_mtp = pd.DataFrame(pd.concat(
                [x_coords[t].iloc[0:2], y_coords[t].iloc[0:2]], axis=1))
            mtp_ankle = pd.DataFrame(pd.concat(
                [x_coords[t].iloc[1:3], y_coords[t].iloc[1:3]], axis=1))
            ankle_knee = pd.DataFrame(pd.concat(
                [x_coords[t].iloc[2:4], y_coords[t].iloc[2:4]], axis=1))
            knee_hip = pd.DataFrame(pd.concat(
                [x_coords[t].iloc[3:5], y_coords[t].iloc[3:5]], axis=1))
            hip_crest = pd.DataFrame(pd.concat(
                [x_coords[t].iloc[4:6], y_coords[t].iloc[4:6]], axis=1))
